...
use crate::gamma::{
	common::{Binder, Closure, Copyability, Level, Name},
	ir::{
...
pub fn close(value: ob::DynamicValue) -> Program {
...
impl Closer {
	fn close(&mut self, value: ob::DynamicValue, is_occurrent: &mut Vec<bool>) -> Term {
		match value {
			// Variables.
			ob::DynamicValue::Variable(_, l) => {
				is_occurrent.get_mut(l.0).map(|has_encountered_l| *has_encountered_l = true);
				Term::Variable(Variable::Local(l))
			}

			// Procedures.
			ob::DynamicValue::Function { base, family: _, body } =>
				todo!(),
				//Term::Function(self.close_function((*base).clone(), body, is_occurrent)),

			// Binding cases.
			ob::DynamicValue::Let { ty, argument, tail } => todo!()/*Term::Let {
			...
			ob::DynamicValue::Pi { base_universe: _, base, family_universe, family } => Term::Pi {
				base: self.close((*base).clone(), is_occurrent).into(),
				family: todo!(),//self.close_function((*base).clone(), family, is_occurrent),
				family_universe,
			},
			ob::DynamicValue::Sigma { base_universe, base, family_universe, family } => Term::Sigma {
				base_universe,
				base: self.close((*base).clone(), is_occurrent).into(),
				family_universe,
				family: todo!(),//self.close_function((*base).clone(), family, is_occurrent),
			...
			// 0-recursive cases.
			ob::DynamicValue::Nat => Term::Nat,
			ob::DynamicValue::Enum(k) => Term::Enum(k),
			ob::DynamicValue::Universe(k) => Term::Universe(k),
			ob::DynamicValue::Num(n) => Term::Num(n),
			ob::DynamicValue::EnumValue(k, v) => Term::EnumValue(k, v),

			// 1-recursive cases.
			ob::DynamicValue::Project(t, p, u) => Term::Project(self.close((*t).clone(), is_occurrent).into(), p, u),
			ob::DynamicValue::Suc(t) => Term::Suc(self.close((*t).clone(), is_occurrent).into()),
			ob::DynamicValue::WrapType(t, _) => Term::WrapType(self.close((*t).clone(), is_occurrent).into()),
			ob::DynamicValue::WrapNew(t) => Term::WrapNew(self.close((*t).clone(), is_occurrent).into()),
			ob::DynamicValue::Unwrap(t, u) => Term::Unwrap(self.close((*t).clone(), is_occurrent).into(), u),
			ob::DynamicValue::RcType(t, _) => Term::RcType(self.close((*t).clone(), is_occurrent).into()),
			ob::DynamicValue::RcNew(t) => Term::RcNew(self.close((*t).clone(), is_occurrent).into()),
			ob::DynamicValue::UnRc(t, u) => Term::UnRc(self.close((*t).clone(), is_occurrent).into(), u),

			// 2-recursive cases
			ob::DynamicValue::Pair { basepoint, fiberpoint } => Term::Pair {
				basepoint: self.close((*basepoint).clone(), is_occurrent).into(),
				fiberpoint: self.close((*fiberpoint).clone(), is_occurrent).into(),
			},
			ob::DynamicValue::Refl => todo!(),

			ob::DynamicValue::Id { kind, space, left, right } => todo!(),

			// Other cases
			ob::DynamicValue::Apply { scrutinee, argument, fiber_universe, base, family } => Term::Apply {
				...
				family: todo!()//(fiber_universe.0 == Copyability::Nontrivial).then(|| self.close_with(family, [(*base).clone()], is_occurrent)),
			},
			ob::DynamicValue::CaseNat { scrutinee, case_nil, case_suc, fiber_universe, motive } => {
				Term::CaseNat {
					...
					case_suc: todo!(), //self.close_with(case_suc, [ob::Term::Nat, (*motive.body).clone()], is_occurrent),
					fiber_representation: fiber_universe.1,
					motive: todo!(), //(fiber_universe.0 == Copyability::Nontrivial).then(|| self.close_with(motive, [ob::Term::Nat], is_occurrent)),
				}
			}
			ob::DynamicValue::CaseEnum { scrutinee, cases, fiber_universe, motive } => {
				let cardinality: u16 = cases.len().try_into().unwrap();
				Term::CaseEnum {
					scrutinee: self.close((*scrutinee).clone(), is_occurrent).into(),
					cases: cases.into_iter().map(|case| self.close(case, is_occurrent)).collect(),
					fiber_representation: fiber_universe.1,
					motive: todo!(), //(fiber_universe.0 == Copyability::Nontrivial).then(|| self.close_with(motive, [ob::Term::Enum(cardinality)], is_occurrent)),
				}
			}
			ob::DynamicValue::CasePath { scrutinee, motive, case_refl } => todo!(),
		}
	}

	fn close_function(
		&mut self,
		base: ob::DynamicValue,
		body: Closure<ob::Environment, ob::DynamicValue>,
		...
		todo!();
		//let body = self.close_with(body, [base.clone()], &mut body_occurrents);
	...
	fn close_with<const N: usize>(
		&mut self,
		binder: Closure<ob::Environment, Rc<ob::DynamicValue>, N>,
		tys: [ob::DynamicValue; N],
		is_occurrent: &mut Vec<bool>,
	) -> Binder<Box<Term>, N> {
		...
		let result = todo!(); //binder.mapv(|_, body| self.close((*body).clone(), is_occurrent));
