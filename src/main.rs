...
fn parse_term_eof(input: &str) -> IResult<&str, RawTerm> {
...
// A 'spine' is a list of terms signalling implicit application. https://dl.acm.org/doi/pdf/10.1145/3408983
					...
					RawEliminator::ProjectFiber => {
						if let Value::Sigma { parameter: _, base: _, family } = total_ty.as_ref() {
							let basepoint_value = {
								...
								eliminators.push(Eliminator::ProjectBase);
								evaluate(context.environment.clone(), Term::Compute { scrutinee: bx!(scrutinee.clone()), eliminators })
							};
...
pub fn is_convertible(context_length: Level, left: Rc<Value>, right: Rc<Value>) -> bool {
	...
	match (left.as_ref(), right.as_ref()) {
		...
		(Pair { basepoint: left_basepoint, fiberpoint: left_fiberpoint }, Pair { basepoint: right_basepoint, fiberpoint: right_fiberpoint }) => {
